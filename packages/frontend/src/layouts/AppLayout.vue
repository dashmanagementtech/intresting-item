...
<template>
  <section class="h-screen flex bg-white overflow-hidden">
    <nav
      ...
      <div class="bg-white p-3 min-lg:flex h-screen flex-col justify-between relative"
        ...
        <div class="flex flex-col gap-1">
          <div class="flex gap-5 items-center justify-center py-5">
            <Logo />
            <h1 class="text-xl font-bold text-primary">Monitora</h1>
          </div>
          <div class="flex flex-col gap-2">
            <template v-for="(link, key) in ROUTES" :key>
              <template v-if="link.acl && link.acl.includes(userRole)">
                <router-link v-if="!link.external" :to="link.uri"
                ...
                <a v-else :href="link.uri" class="flex items-center gap-3 p-3 rounded hover:text-primary text-sm"
                  target="_blank">
                  <component :is="link.icon" class="w-[20px]" />
                  {{ link.title }}
                </a>
          ...
          <div class="py-10 flex flex-col gap-3">
            <div class="flex items-center justify-between">
              <h2 class="uppercase text-gray-200 text-xs">active projects</h2>
              <router-link :to="{ name: 'projects' }" class="text-xs text-blue-500">
                See all
              </router-link>
            </div>
            <div v-if="activeProjects.loading" class="text-xs" v-loading="activeProjects.loading">
              Loading active projects and tasks count.
            </div>
            <div v-else-if="activeProjects.error"
              class="bg-red-100 rounded p-3 text-sm flex flex-col items-start gap-2">
              <h3 class="text-red-500 font-semibold">An error occurred!</h3>
            ...
            <el-scrollbar v-else class="!h-[25vh] overflow-y-scroll">
              <div class="flex flex-col gap-2">
                <router-link v-for="project in activeProjects.projects" :key="project.id"
                  ...
                  <div class="h-5 w-5 rounded-full bg-gray-50 flex items-center justify-center"
        ...
        <div class="absolute bottom-0 right-0 left-0 grid grid-cols-2 bg-gray-50 border-t border-gray-50/50">
          <router-link :to="{ name: 'invite-staff' }" class="flex items-center justify-center gap-3 text-sm bg-white hover:bg-transparent mr-0.5">
    ...
    <section class="h-screen w-full bg-white px-5 overflow-hidden">
      <header
        ...
        <div class="flex items-center justify-center gap-5">
          <svg class="w-[24px] cursor-pointer transition-all ease-in-out min-lg:hidden"
